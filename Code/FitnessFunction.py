...
def fitness(cube_orientation):
	...
	# If the cube is solved then the solution will be just ['']
	# If we use len(['']) then it will return 1
	# But it should return 0 since the fitness of the solved state is 0

	if solution == ['']:
		return 0
	
	# If it is note solved return the solved state
	else:
		return len(solution)