...
def ReturnMove(legal_moves,krill_AllMoves,krill_fitness,krill,nK):
	...
	for m in range(len(legal_moves)):
		...
		if fitnessValue < threshold or cube_orientation == "yyyyyyyyyooooooooogggggggggwwwwwwwwwrrrrrrrrrbbbbbbbbb":
			...
			# We will let this loop run till the end, 
			# in case we find another move which has fitness less than the current one
...
def Reproduction(krill1, krill2, krill_OrientationMoves, originalScramble, krill_fitness, krill_Orientation, krill_AllMoves, solution, iteration,nPop,start_time,nK):
	"""
	krill1 - first krill
	krill2 - second krill
	...
	nK - number of times the evalaution function is to be called
	"""
...
def Mutation(fitnessThreshold, nPop, krill_fitness, krill_AllMoves, krill_Orientation, krill_OrientationMoves, legal_moves,originalScramble,start_time,solution, iteration,nK):
	...
	solution - the data structure holding the solution, 
	iteration - iteration at which the mutation was called.
	nK - number of times the Kociemba's algorithm is to be called
	"""
...
# Function definition
def KHO(iterations, krills, flag, scramble, nk):
	...
	for iteration in range(maxIter):
		...
		for krill in range(nPop):
			...
			# If the krill has not reached the solved state.
			if krill_Orientation[krill] != "yyyyyyyyyooooooooogggggggggwwwwwwwwwrrrrrrrrrbbbbbbbbb":
			
				# Now select the next egde/move for the krill and find its fitness
				move, fitnessValue = ReturnMove(legal_moves,krill_AllMoves,krill_fitness,krill,nK)
				...
				# No need to find fitness again, since we have found it when we selected the move to 
			...
			while nPop != originalPopulation:
				...
				for i in range(len(krill_OrientationMoves)):
					...
					for j in range(len(krill_OrientationMoves)):
						...
						Reproduction(i, j, krill_OrientationMoves, originalScramble, krill_fitness, krill_Orientation, krill_AllMoves, solution, iteration,nPop, start_time,nK)
						nPop += 1
						...
						Reproduction(j, i, krill_OrientationMoves, originalScramble, krill_fitness, krill_Orientation, krill_AllMoves, solution, iteration,nPop,start_time,nK)
						nPop += 1
		...
		Mutation(fitnessThreshold, nPop, krill_fitness, krill_AllMoves, krill_Orientation, krill_OrientationMoves,legal_moves, originalScramble,start_time,solution, iteration,nK)
...
if __name__ == "__main__":
	...
	KHO(iterations, krills, flag, scramble+" ",nK)	# Calling the DKHO algorithm